...
<div id="page-wrapper">
	...
	if(isset($_SESSION['cloak_to'])){ ?>
		...
		<?php
				...
				$zapytanie=$db->query("SELECT * FROM szablony WHERE id='$id_szablonu'");
		...
		<div>
				...
				<?php 
					...
					echo "<img src='image/$record->grafika;' alt='szablon' style='max-height:300px; display: block; margin-left: auto; margin-right: auto;'/>"; 
				...
				if(isset($_POST['submit']))
				{
						...
						$sprawdzenie=$db->query("SELECT * FROM nazwa_cv WHERE konto='$jakie_konto' and nazwa='$nazwa_cv'");
						...
						if($policz==0)
						{
						$dodaj=$db->query("INSERT INTO nazwa_cv (id, konto, nazwa, id_szablon, dodano) VALUES (NULL, '$jakie_konto', '$nazwa_cv', '$id_szablonu', '$data')");
						
							if($dodaj==true)
							{
								header("Location: nowe_cv_dodaj_dane.php?nazwa_cv=$nazwa_cv&id_szablonu=$id_szablonu&data=$data");
							}
						}
						else
						{
